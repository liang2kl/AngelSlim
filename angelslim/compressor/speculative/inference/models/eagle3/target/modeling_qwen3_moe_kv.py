...
# Source: src/transformers/models/qwen3_moe/modeling_qwen3_moe.py (Old Version)
#
# Modifications made for AngelSlim/EAGLE 3 project:
...
_CHECKPOINT_FOR_DOC = "Qwen/Qwen3-MoE"
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_values is not None:
            # EAGLE / AngelSlim modification: Direct concatenation for preallocated buffers
        ...
        # Avoid returning past_key_values in the tuple if we handle it manually
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Check if MoE (returns tuple) or Dense (returns tensor)
...
class Qwen3MoeModel(Qwen3MoePreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # EAGLE 3 Modification: Use custom mask update logic for tree_mask support
        ...
        for idx, decoder_layer in enumerate(
            ...
            # Handling return indices based on output configuration
            # Standard return: (hidden_states, [attentions], [cache], [router_logits])
            cache_idx = 1
            if output_attentions:
                cache_idx += 1
            ...
            if output_router_logits:
                # router logits are always the last element if present
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if sequence_length == target_length:
                ...
                # EAGLE/AngelSlim Tree Mask Support
...
@add_start_docstrings(
    "The Qwen3 MoE Model transformer with a language modeling head on top (linear layer with weights tied to the input embeddings).",
    QWEN3_MOE_START_DOCSTRING,
)
...
class Qwen3MoeForSequenceClassification(Qwen3MoePreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            last_non_pad_token = -1
            logger.warning_once(
                f"{self.__class__.__name__} will not detect padding tokens in `inputs_embeds`. Results may be unexpected."
            )
